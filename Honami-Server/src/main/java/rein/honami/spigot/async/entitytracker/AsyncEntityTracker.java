package rein.honami.spigot.async.entitytracker;
...
public class AsyncEntityTracker extends EntityTracker {
	...
	private static final ExecutorService trackingThreadExecutor = Executors.newCachedThreadPool(new ThreadFactoryBuilder().setNameFormat("Honami Entity Tracker Thread").build());
	...
	public void updatePlayers() {	
		...
		for (int i = 1; i <= HonamiConfig.trackingThreads; i++) {
			final int finalOffset = offset++;
			
			AsyncUtil.run(() -> {
				for (int index = finalOffset; index < c.size(); index += HonamiConfig.trackingThreads) {
