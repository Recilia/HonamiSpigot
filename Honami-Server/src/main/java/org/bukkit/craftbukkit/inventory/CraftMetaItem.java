...
class CraftMetaItem implements ItemMeta, Repairable {
	...
	CraftMetaItem(CraftMetaItem meta) {
		...
		spigot.setUnbreakable(meta.spigot.isUnbreakable()); 
	}
	...
	CraftMetaItem(NBTTagCompound tag) {
		if (tag.hasKey(DISPLAY.NBT)) {
			...
			if (display.hasKey(NAME.NBT)) {
				displayName = limit(display.getString(NAME.NBT), 1024); 
			}
			...
			if (display.hasKey(LORE.NBT)) {
				...
				for (int index = 0; index < list.size(); index++) {
					String line = limit(list.getString(index), 1024); 
					lore.add(line);
				}
			}
		}
	...
	boolean isEmpty() {
		return !(hasDisplayName() || hasEnchants() || hasLore() || hasRepairCost() || !unhandledTags.isEmpty()
				|| hideFlag != 0 || spigot.isUnbreakable()); 
	}
	...
	int applyHash() {
		...
		hash = 61 * hash + (spigot.isUnbreakable() ? 1231 : 1237); 
		return hash;
	}
	...
	public static Set<String> getHandledTags() {
		synchronized (HANDLED_TAGS) {
			if (HANDLED_TAGS.isEmpty()) {
				HANDLED_TAGS.addAll(Arrays.asList(UNBREAKABLE.NBT, 
