	...
	public void b() {
		for (int i = 0; i < this.c.size(); ++i) {
			...
			if (!ItemStack.fastMatches(itemstack1, itemstack)
					|| (tickCount % 20 == 0 && !ItemStack.matches(itemstack1, itemstack))) { 
			...
			if ((i1 != 1 || this.g != 2) && i1 != this.g) {
				...
				case 2:
					if (!this.h.isEmpty()) {
						...
						Map<Integer, ItemStack> draggedSlots = new HashMap<Integer, ItemStack>(); 
						...
						while (iterator.hasNext()) {
							...
							if (slot1 != null && a(slot1, playerinventory.getCarried(), true)
								...
								draggedSlots.put(slot1.rawSlotIndex, itemstack2); 
