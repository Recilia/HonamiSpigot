...
public abstract class EntityInsentient extends EntityLiving {
	...
	private WeakReference<EntityLiving> goalTarget; 
	...
	private boolean shouldBreakLeash = true; 
	private boolean pullWhileLeashed = true; 
	...
	protected final void doTick() {
		...
		this.navigation.cleanUpExpiredSearches(); 
		...
		if (this.bo && this.shouldBreakLeash) {
			if (!this.isAlive()) {
				this.world.getServer().getPluginManager()
						.callEvent(new EntityUnleashEvent(this.getBukkitEntity(), UnleashReason.PLAYER_UNLEASH)); 
				this.unleash(true, true);
			}

			if (this.bp == null || this.bp.dead) {
				this.world.getServer().getPluginManager()
						.callEvent(new EntityUnleashEvent(this.getBukkitEntity(), UnleashReason.HOLDER_GONE)); 
				this.unleash(true, true);
			}
		}
	}
	...
	private void n() {
		if (this.bo && this.bq != null) {
			if (this.bq.hasKeyOfType("UUIDMost", 4) && this.bq.hasKeyOfType("UUIDLeast", 4)) {
				...
				this.world.getServer().getPluginManager()
						.callEvent(new EntityUnleashEvent(this.getBukkitEntity(), UnleashReason.UNKNOWN)); 
