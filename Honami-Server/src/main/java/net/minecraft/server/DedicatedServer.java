...
import rein.honami.spigot.commons.IPUtils;
...
public class DedicatedServer extends MinecraftServer implements IMinecraftServer {
	...
	private final java.util.Queue<ServerCommand> l = new java.util.concurrent.ConcurrentLinkedQueue<>(); 
    ...
    public DedicatedServer(joptsimple.OptionSet options) {
		...
		if (!HonamiConfig.disableInfiniSleeperThreadUsage) {
	...
	protected boolean init() throws IOException {
		...
		if (!this.p.a() && !eulaAgreed) { 
			...
			HonamiConfig.init((File) options.valueOf("honami-settings"));
			...
			rein.honami.spigot.via.ViaBootstrap.extract((File) options.valueOf("plugins")); 
			server.loadPlugins();
			...
			if (!this.getOnlineMode()) {
				...
				if (!HonamiConfig.stopNotifyBungee) {
					DedicatedServer.LOGGER
							.warn("---------------------------- Honami Checker ----------------------------");
						DedicatedServer.LOGGER.warn(
								"If you don't want to see this message anymore, set \"settings.stop-notify-bungee\" to \"true\" in \"honami.yml\"!");
	...
	protected boolean aR() {
		server.getLogger().info("**** Beginning UUID conversion, this may take A LONG time ****"); 
