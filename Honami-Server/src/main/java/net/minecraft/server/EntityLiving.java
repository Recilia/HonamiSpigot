...
import rein.honami.api.event.RefinePotionExpireEvent;
import rein.honami.api.event.RefinePotionAddEvent;
import rein.honami.api.event.player.PlayerHealthChangeEvent;
...
public abstract class EntityLiving extends Entity {
		...
		while (iterator.hasNext()) {
			...
			if (!mobeffect.tick(this)) {
				if (!this.world.isClientSide) {
					if (this instanceof EntityPlayer) {
						RefinePotionExpireEvent expireEvent = new RefinePotionExpireEvent(
								((EntityPlayer) this).getBukkitEntity(), mobeffect.getEffectId(), mobeffect.getAmplifier());
	...
	public void addEffect(MobEffect mobeffect) {
		org.spigotmc.AsyncCatcher.catchOp("effect add"); 
		...
		if (this.d(mobeffect)) {
			if (this instanceof EntityPlayer) {
				RefinePotionAddEvent addEvent = new RefinePotionAddEvent(
						((EntityPlayer) this).getBukkitEntity(), mobeffect.getEffectId(), mobeffect.getAmplifier());
	...
	public void setHealth(float f) {
		...
		if (this instanceof EntityPlayer) {
			...
			PlayerHealthChangeEvent healthEvent = new PlayerHealthChangeEvent(
					player, oldHealth, f, player.getMaxHealth());
			Bukkit.getPluginManager().callEvent(healthEvent);
			if (healthEvent.isCancelled()) {
				return;
			}
			f = (float) healthEvent.getNewHealth();
	...
	protected void bL() {
		...
		if (this.ad() && !list.isEmpty()) { 
			numCollisions -= world.spigotConfig.maxCollisionsPerEntity; 
	...
	public ScoreboardTeamBase getScoreboardTeam() {
		if (!this.world.honamiTacoConfig.nonPlayerEntitiesOnScoreboards && !(this instanceof EntityHuman)) {
			return null; 
		}
