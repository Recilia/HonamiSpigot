...
import rein.honami.spigot.nacho.config.NachoWorldConfig;
import rein.honami.spigot.playermap.PlayerMap;
...
public abstract class World implements IBlockAccess {
	...
	protected final Set<Entity> g = Sets.newHashSet(); 
	...
	public ExecutorService lightingExecutor = Executors
			.newSingleThreadExecutor(new ThreadFactoryBuilder().setNameFormat("PaperSpigot - Lighting Thread").build()); 

																															
																															
	public java.util.ArrayDeque<BlockRedstoneTorch.RedstoneUpdateInfo> redstoneUpdateInfos; 
	...
	public final org.github.paperspigot.PaperSpigotWorldConfig paperSpigotConfig; 

	public final co.aikar.timings.WorldTimingsHandler timings; 
	public final rein.honami.spigot.taco.HonamiTacoWorldConfig honamiTacoConfig; 
	public final NachoWorldConfig nachoSpigotConfig; 
	...
	protected World(IDataManager idatamanager, WorldData worlddata, WorldProvider worldprovider,
		...
		this.spigotConfig = new org.spigotmc.SpigotWorldConfig(worlddata.getName()); 
		this.paperSpigotConfig = new org.github.paperspigot.PaperSpigotWorldConfig(worlddata.getName()); 
		this.honamiTacoConfig = new rein.honami.spigot.taco.HonamiTacoWorldConfig(worlddata.getName()); 
		this.nachoSpigotConfig = new NachoWorldConfig(worlddata.getName()); 
		this.generator = gen;
		...
		this.chunkTickRadius = (byte) ((spigotConfig.viewDistance < 7) ? spigotConfig.viewDistance : 7); 
	...
	public boolean isLoaded(int blockposition_x, int blockposition_y, int blockposition_z) {
		return getChunkIfLoaded(blockposition_x >> 4, blockposition_z >> 4) != null; 
	...
	public boolean setTypeAndData(BlockPosition blockposition, IBlockData iblockdata, int i, boolean updateLight) { 
		...
		if (!this.isValidLocation(blockposition)) {
			...
			IBlockData iblockdata1 = chunk.a(blockposition, iblockdata, updateLight); 
	...
	public void a(int i, int j, int k, int l) {
		...
		if (!this.worldProvider.o()) {
			for (i1 = k; i1 <= l; ++i1) {
				this.updateLight(EnumSkyBlock.SKY, new BlockPosition(i, i1, j)); 
	...
	public void applyPhysics(BlockPosition blockposition, Block block) {
		...
		spigotConfig.antiXrayInstance.updateNearbyBlocks(this, blockposition); 
	}
	...
	public void d(BlockPosition blockposition, final Block block) {
		if (!this.isClientSide) {
			...
			try {
				...
				if (world != null && (this.honamiTacoConfig.isRedstoneFireBPE || !(block instanceof BlockRedstoneWire
	...
	public boolean addEntity(Entity entity, SpawnReason spawnReason) { 
		...
		if (!flag && !this.isChunkLoaded(i, j, true)) {
			...
			if (entity instanceof EntityLiving) {
				((EntityLiving) entity).maxNoDamageTicks = HonamiConfig.hitDelay;
			}

			
			if (entity instanceof EntityPotion) {
				
				entity.motY -= HonamiConfig.potionSpeed;	

				if (!(HonamiConfig.potionSpeed == 0)) {
	...
	public void removeEntity(Entity entity) {
		org.spigotmc.AsyncCatcher.catchOp("entity remove"); 
		entity.die();
		if (entity instanceof EntityHuman) {
			this.players.remove(entity);
			this.playerMap.remove((EntityPlayer) entity);
	...
	public void tickEntities() {
		...
		co.aikar.timings.TimingHistory.entityTicks += this.entityList.size(); 
		...
		co.aikar.timings.TimingHistory.tileEntityTicks += this.tileEntityList.size(); 
	...
	protected void D() {
		...
		if (optimalChunks > 0) {
			...
			for (i = 0; i < this.players.size(); ++i) {
				...
				int existingPlayers = Math.max(0, chunkTickList.get(key)); 
				...
				for (int chunk = 0; chunk < chunksPerPlayer; chunk++) {
					...
					if (!chunkTickList.containsKey(hash) && this.chunkProvider.isChunkLoaded(dx + j, dz + k)) { 
						chunkTickList.put(hash, (short) -1); 
					}
				}
			}
	...
	public boolean x(BlockPosition blockposition) {
		...
		if (!this.worldProvider.o()) {
			flag |= this.updateLight(EnumSkyBlock.SKY, blockposition); 
		}

		flag |= this.updateLight(EnumSkyBlock.BLOCK, blockposition); 
		return flag;
	}
	...
	public void b(Collection<Entity> collection) {
		org.spigotmc.AsyncCatcher.catchOp("entity world add"); 
