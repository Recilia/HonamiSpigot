...
import rein.honami.spigot.world.WorldTicker;
...
public class WorldServer extends World  { 
	...
	public WorldServer(MinecraftServer minecraftserver, IDataManager idatamanager, WorldData worlddata, int i,
		...
		this.Q = ((org.bukkit.craftbukkit.CraftTravelAgent) new org.bukkit.craftbukkit.CraftTravelAgent(this)
				.setSearchRadius(paperSpigotConfig.portalSearchRadius)); 
	...
	public void doTick() {
		...
		spigotConfig.antiXrayInstance.flushUpdates(this); 
	...
	protected void e() {
		this.O = false;
		Iterator iterator = this.players.stream().filter(EntityHuman::isSleeping).iterator(); 
		...
		if (this.worldData.getType() == WorldType.DEBUG_ALL_BLOCK_STATES) {
			...
			for (it.unimi.dsi.fastutil.objects.ObjectIterator<it.unimi.dsi.fastutil.longs.Long2ShortMap.Entry> iter = chunkTickList.long2ShortEntrySet().fastIterator(); iter.hasNext();) { 
				...
				if ((!this.chunkProvider.isChunkLoaded(chunkX, chunkZ))
						|| (this.chunkProviderServer.unloadQueue.contains(LongHash.toLong(chunkX, chunkZ)))) 
	...
	public boolean a(boolean flag) {
		if (this.worldData.getType() == WorldType.DEBUG_ALL_BLOCK_STATES) {
			...
			timings.scheduledBlocksCleanup.stopTiming(); 
			...
			timings.scheduledBlocksTicking.startTiming(); 
			...
			while (iterator.hasNext()) {
				...
				if (this.areChunksLoadedBetween(nextticklistentry.a.a(-b0, -b0, -b0),
						nextticklistentry.a.a(b0, b0, b0))) {
					IBlockData iblockdata = this.getType(nextticklistentry.a);
					co.aikar.timings.Timing timing = iblockdata.getBlock().getTiming(); 
					timing.startTiming(); 
			...
			timings.scheduledBlocksTicking.stopTiming(); 
