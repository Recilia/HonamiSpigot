...
import rein.honami.api.event.EquipmentSetEvent;
import rein.honami.api.event.PlayerResetCooldownEvent;
import rein.honami.api.event.PlayerResetHitCooldownEvent;

import com.google.common.base.Charsets;
import com.google.common.collect.Lists;
import com.mojang.authlib.GameProfile;
import rein.honami.spigot.event.HonamiCombatHitEvent;
import rein.honami.spigot.event.HonamiEventBus;
import rein.honami.spigot.event.HonamiKnockbackEvent;
...
public abstract class EntityHuman extends EntityLiving {
	...
	public void m() {
		...
		float f1 = (float) (org.bukkit.craftbukkit.TrigMath.atan(-this.motY * 0.20000000298023224D) * 15.0D); 
			...
			if (!entity.l(this)) {
				...
				if (f > 0.0F || f1 > 0.0F) {
					boolean flag = !world.paperSpigotConfig.disablePlayerCrits && this.fallDistance > 0.0F
							...
							&& this.vehicle == null && entity instanceof EntityLiving; 
					...
					if (flag2) {
						...
						Bukkit.getPluginManager().callEvent(new PlayerResetCooldownEvent((Player) this.getBukkitEntity()));
						Bukkit.getPluginManager().callEvent(new PlayerResetHitCooldownEvent((Player) this.getBukkitEntity()));
						}
						
						if (HonamiConfig.optimizedEventBus && this.getBukkitEntity() instanceof Player
								&& entity.getBukkitEntity() instanceof Player) {
							HonamiEventBus.getInstance().fire(new HonamiCombatHitEvent(
									(Player) this.getBukkitEntity(), (Player) entity.getBukkitEntity(), f, flag));
						}
						...
						if (i > 0) {
							...
							if (entity.getBukkitEntity() instanceof Player) {
								KnockbackProfile worldKb = WorldCombatManager.getKnockbackProfileForPlayer(
						...
						if (entity instanceof EntityPlayer && entity.velocityChanged) {
							...
							if (!event.isCancelled()) {
								...
								if (HonamiConfig.optimizedEventBus && this.getBukkitEntity() instanceof Player) {
									HonamiEventBus.getInstance().fire(
											new HonamiKnockbackEvent((Player) this.getBukkitEntity(), player, velocity));
								}
							}
	...
	public void bF() {
		...
		if (this.isSprinting()) {
			this.applyExhaustion(world.spigotConfig.sprintExhaustion); 
		} else {
			this.applyExhaustion(world.spigotConfig.walkExhaustion); 
		}
		...
		if (this.vehicle == null) {
			...
			if (this.a(Material.WATER)) {
				...
				if (i > 0) {
					...
					this.applyExhaustion(world.paperSpigotConfig.playerSwimmingExhaustion * i * 0.01F); 
	...
	public void setEquipment(int i, ItemStack itemstack) {
		ItemStack oldItem = this.inventory.armor[i];
		this.inventory.armor[i] = itemstack;
		if (this.getBukkitEntity() instanceof Player) {
			EquipmentSetEvent equipmentEvent = new EquipmentSetEvent(
					(Player) this.getBukkitEntity(), i,
					oldItem != null ? CraftItemStack.asBukkitCopy(oldItem) : null,
					itemstack != null ? CraftItemStack.asBukkitCopy(itemstack) : null);
			Bukkit.getPluginManager().callEvent(equipmentEvent);
		}
	}
